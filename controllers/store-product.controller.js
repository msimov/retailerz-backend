...
exports.create = (req, res) => {
  ...
  StoreProduct.create(req.params.storeId, req.body, (err, data) => {
    if (err) {
      res.status(500).send({
...
exports.getAllByStoreId = (req, res) => {
  StoreProduct.getAllByStoreId(req.params.storeId, (err, data) => {
    if (err) {
      res.status(500).send({
        message: err.message || "Error.",
      });
...
exports.getAllByProductId = (req, res) => {
  StoreProduct.getAllByProductId(req.params.productId, (err, data) => {
    if (err) {
      res.status(500).send({
        message: err.message || "Error.",
      });
