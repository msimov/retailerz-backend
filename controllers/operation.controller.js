...
exports.create = (req, res) => {
  ...
  Operation.create(req.params.userId, req.body, (err, data) => {
    if (err) {
      res.status(500).send({
...
exports.findByOperationId = (req, res) => {
  Operation.findByOperationId(req.params.operationId, (err, data) => {
    if (err) {
...
exports.getAllByUserId = (req, res) => {
  Operation.getAllByUserId(req.params.userId, (err, data) => {
    if (err) {
      res.status(500).send({
        message: err.message || "Error.",
      });
...
exports.getInventory = (req, res) => {
  Operation.getInventory(req.params.userId, (err, data) => {
    if (err) {
      res.status(500).send({
        message: err.message || "Error.",
      });
...
exports.deleteByOperationId = (req, res) => {
  Operation.deleteByOperationId(req.params.operationId, (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        res.status(404).send({
          message: "Error.",
