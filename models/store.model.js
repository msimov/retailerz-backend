const {
  ...
  UPDATE_BY_STORE_ID,
  DELETE_BY_STORE_ID,
} = require("../constants/store.constants");
...
Store.findByStoreId = (storeId, result) => {
  sql.query(FIND_BY_STORE_ID, storeId, (err, res) => {
    if (err) {
      result(err, null);
      return;
    }
...
Store.getAllByUserId = (storeUserId, result) => {
  sql.query(GET_ALL_BY_USER_ID, storeUserId, (err, res) => {
    if (err) {
      result(err, null);
      return;
    }
...
Store.deleteByStoreId = (storeId, result) => {
  sql.query(DELETE_BY_STORE_ID, storeId, (err, res) => {
    if (err) {
      result(err, null);
      return;
    }
