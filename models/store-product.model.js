...
} = require("../constants/store-product.constants");
...
StoreProduct.getAllByStoreId = (storeProductStoreId, result) => {
  sql.query(GET_ALL_BY_STORE_ID, storeProductStoreId, (err, res) => {
    if (err) {
      result(err, null);
      return;
    }
...
StoreProduct.getAllByProductId = (storeProductProductId, result) => {
  sql.query(GET_ALL_BY_PRODUCT_ID, storeProductProductId, (err, res) => {
    if (err) {
      result(err, null);
      return;
    }
