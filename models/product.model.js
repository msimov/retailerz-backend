const {
  ...
  GET_ALL_RECOMMENDED_BY_USER_ID,
...
Product.findByProductId = (productId, result) => {
  sql.query(FIND_BY_PRODUCT_ID, productId, (err, res) => {
    if (err) {
      result(err, null);
      return;
    }
...
Product.search = (search, result) => {
  sql.query(SEARCH(search), (err, res) => {
    if (err) {
      result(err, null);
      return;
    }
...
Product.getAllRecommendedByUserId = (userId, result) => {
  sql.query(GET_ALL_RECOMMENDED_BY_USER_ID(userId), (err, res) => {
    if (err) {
      result(err, null);
      return;
    }
...
Product.deleteByProductId = (productId, result) => {
  sql.query(DELETE_BY_PRODUCT_ID, productId, (err, res) => {
    if (err) {
      result(err, null);
      return;
    }
