...
} = require("../constants/measure-unit.constants");
...
MeasureUnit.findByMeasureUnitId = (measureUnitId, result) => {
  sql.query(FIND_BY_MEASURE_UNIT_ID, measureUnitId, (err, res) => {
    if (err) {
      result(err, null);
      return;
    }
...
MeasureUnit.getAllByUserId = (measureUnitUserId, result) => {
  sql.query(GET_ALL_BY_USER_ID, measureUnitUserId, (err, res) => {
    if (err) {
      result(err, null);
      return;
    }
...
MeasureUnit.deleteByMeasureUnitId = (measureUnitId, result) => {
  sql.query(DELETE_BY_MEASURE_UNIT_ID, measureUnitId, (err, res) => {
    if (err) {
      result(err, null);
      return;
    }
